...
def double_a_float():
    x = float(input("Please provide your float here:"))
    print(x*2)
    """
...
def print_an_integer_many_times():
    x = int(input("Please provide and interger:"))
    y = x * 2
    for k in range(x):
        print(y)
...
def input_it_all():
    x = float(input('Please provide a float:'))
    y = int(input('Please provide an interger:'))
    z = str(input('Please provide a statement:'))
    a = math.sqrt(x)
    for k in range(y):
        print(a)
    for j in range(y):
